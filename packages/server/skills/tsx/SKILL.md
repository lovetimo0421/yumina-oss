...
| `getPersonaProfile()` | Explicitly import the current session's selected Persona: returns a Promise of `{name, appearance, personality, backstory, entries?: Array<{title, content}>}` or `null` when none is selected; failures reject. Reads the same saved selection displayed by the Persona manager, including an explicit No Persona; never substitutes account defaults. Private notes are excluded. Call when the player chooses import, allow review/editing, and save a copy per run if identity must stay fixed. Older hosts may not expose this method; offer manual entry as a fallback. |
