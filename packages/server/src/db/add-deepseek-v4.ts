/**
 * Idempotent migration — add the two new DeepSeek V4 models to the
 * `model_prices` table without disturbing existing rows.
 *
 * Run with: tsx packages/server/src/db/add-deepseek-v4.ts
 *
 ...
 *
 * Pricing fallback (raw upstream, $ per million tokens):
 *   - deepseek/deepseek-v4-flash : $0.14 in / $0.28 out  (budget tier, 1.0× markup)
 *   - deepseek/deepseek-v4-pro   : $0.435 in / $0.87 out (standard tier, 1.1× markup)
 * Live official traffic bills from OpenRouter's returned usage.cost instead.
 */
...
const NEW_MODELS: ModelRow[] = [
  { modelId: "deepseek/deepseek-v4-flash", inputPricePerM: 0.14,  outputPricePerM: 0.28, minPlan: "free", markup: 1.0 },
  { modelId: "deepseek/deepseek-v4-pro",   inputPricePerM: 0.435, outputPricePerM: 0.87, minPlan: "free", markup: 1.1 },
];

async function main() {
  console.log("=== add-deepseek-v4: upserting DeepSeek V4 model prices ===\n");
  ...
  const verify = await db.execute(sql`
    ...
    FROM model_prices
    WHERE model_id IN ('deepseek/deepseek-v4-flash', 'deepseek/deepseek-v4-pro')
    ORDER BY model_id
  `);
  console.log("\nFinal state:");
